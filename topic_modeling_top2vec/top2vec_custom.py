...
class Top2Vec:
    ...
    args: dict (Optional, default None)
        Pass custom arguments to HDBSCAN or DBSCAN.
    ...
    def __init__(self,
                 ...
                 topic_metric='cosine_similarity',
                 ):

        self.doc_df = pd.DataFrame({"Document": documents,
        ...
        if document_ids is not None:
            if not (isinstance(document_ids, list) or isinstance(document_ids, np.ndarray)):
                raise ValueError(
                    "Documents ids need to be a list of str or int")
        ...
        if embedding_model == 'doc2vec':
            ...
            if use_corpus_file:
                ...
                train_corpus = [TaggedDocument(
                    tokenizer(doc), [i]) for i, doc in enumerate(documents)]
            ...
            vectorizer = CountVectorizer(
                tokenizer=return_doc, preprocessor=return_doc)
            ...
            self.word_vectors = self._l2_normalize(
                np.array(self.embed(self.vocab)))

            # embed documents
            if use_embedding_model_tokenizer:
                self.document_vectors = self._embed_documents(
                    documents, dataset_type, use_cached)
            else:
                train_corpus = [' '.join(tokens)
                                for tokens in tokenized_corpus]
                self.document_vectors = self._embed_documents(
                    train_corpus, dataset_type, use_cached)

        else:
            raise ValueError(
                f"{embedding_model} is an invalid embedding model.")
        ...
        umap_model = umap.UMAP(
            **umap_args).fit(self._get_document_vectors(norm=False))

        # find dense areas of document vectors
        logger.info('Finding dense areas of documents')

        if args is None:
            args = {'min_cluster_size': 15,
                    'metric': 'euclidean',
                    'cluster_selection_method': 'eom'}

        self.umap_cls = umap_model

        if clustering_model == 'dbscan':
            cluster = DBSCAN(**args).fit(umap_model.embedding_)
        elif clustering_model == 'hdbscan':
            cluster = hdbscan.HDBSCAN(**args).fit(umap_model.embedding_)
#         cluster = KMeans(**hdbscan_args).fit(umap_model.embedding_)

        self.cluster_cls = cluster

        self.doc_df['Topic'] = cluster.labels_
        ...
        self.doc_top, self.doc_dist, self.doc_dist_all = self._calculate_documents_topic(self.topic_vectors,
                                                                                         self._get_document_vectors(), metric=topic_metric)
    ...
    def _embed_documents(self, train_corpus, dataset_type, use_cached):

        if use_cached:
            s = './embedding_cache/' + self.embedding_model + '_' + dataset_type + '.npy'
            if path.exists(s):
                print('Loading file from ' + s)
                document_vectors = np.load(
                    './embedding_cache/' + self.embedding_model + '_' + dataset_type + '.npy')
                return document_vectors
            else:
                print('No cache exists! Creating a new cache')
        ...
        for ind in range(0, batches):
            document_vectors.append(self.embed(
                train_corpus[current:current + batch_size]))
            current += batch_size

        if extra > 0:
            document_vectors.append(self.embed(
                train_corpus[current:current + extra]))

        document_vectors = self._l2_normalize(
            np.array(np.vstack(document_vectors)))    
        np.save('./embedding_cache/' + self.embedding_model +
                '_' + dataset_type + '.npy', document_vectors)
        return document_vectors
    ...
    def _reorder_topics(self, hierarchy=False):
        # Modified to reorder document topic scores numpy array based on old2new
        ...
        if hierarchy:
            ...
            old2new = dict(zip(self.topic_sizes_reduced.index, range(
                self.topic_sizes_reduced.index.shape[0])))
            self.doc_top_reduced = np.array(
                [old2new[i] for i in self.doc_top_reduced])
            self.hierarchy = [self.hierarchy[i]
                              for i in self.topic_sizes_reduced.index]
            ...
            old2new = dict(zip(self.topic_sizes.index, range(
                self.topic_sizes.index.shape[0])))
            self.mapping = old2new
            self.doc_top = np.array([old2new[i] for i in self.doc_top])

            self.doc_dist_all_ordered = []  # Added lines 664-669
            for it, items in enumerate(self.doc_dist_all):
                tmp = {}
                for jt, items_single in enumerate(self.doc_dist_all[it]):
                    tmp[old2new[jt]] = items_single
                self.doc_dist_all_ordered.append(tmp)
            self.topic_sizes.reset_index(drop=True, inplace=True)

    def _reorder_topics_pred(self):
        #         self.doc_top_test = np.array([self.mapping[i] for i in self.doc_top_test])
        #         print(self.doc_top_test, '\n')
        self.doc_dist_all_ordered_test = []  # Added lines 664-669
        for it, items in enumerate(self.doc_dist_test_all):
            tmp = {}
            for jt, items_single in enumerate(self.doc_dist_test_all[it]):
                tmp[jt] = items_single
            self.doc_dist_all_ordered_test.append(tmp)

    @staticmethod
    def _calculate_documents_topic(topic_vectors, document_vectors, metric='cosine_similarity', dist=True):
        # Added doc_dist_all to store document scores for all topics
        batch_size = 10000
        doc_top = []
        doc_dist_all = []  # Added
        if dist:
        ...
        if document_vectors.shape[0] > batch_size:
            ...
            if extra > 0:
                ...
                if dist:
                    doc_dist.extend(np.max(res, axis=1))
            if dist:
                doc_dist_all = np.array(doc_dist_all)
                doc_dist = np.array(doc_dist)
        else:
            #             res = np.inner(document_vectors, topic_vectors)
            res = sim_metric[metric](document_vectors, topic_vectors)
            doc_dist_all.extend(res)  # Added
        ...
        if dist:
            return doc_top, doc_dist, doc_dist_all  # Modified
        else:
            return doc_top
    ...
    def _assign_documents_to_topic(self, document_vectors, hierarchy=False):
        ...
        if hierarchy:
            ...
            self.doc_dist_reduced = np.append(
                self.doc_dist_reduced, doc_dist_new)
    ...
    def _unassign_documents_from_topic(self, doc_indexes, hierarchy=False):
        if hierarchy:
            ...
            self.doc_top_reduced = np.delete(
                self.doc_top_reduced, doc_indexes, 0)
            self.doc_dist_reduced = np.delete(
                self.doc_dist_reduced, doc_indexes, 0)
    ...
    def _validate_hierarchical_reduction(self):
        if self.hierarchy is None:
            raise ValueError(
                "Hierarchical topic reduction has not been performed.")
    ...
    def add_documents(self, documents, doc_ids=None, tokenizer=None, use_embedding_model_tokenizer=False):
        ...
        if self.documents_indexed:
            ...
            new_index_ids = list(
                range(start_index_id, start_index_id + len(doc_ids)))
    ...
    def hierarchical_topic_reduction(self, num_topics):
        ...
        top_sizes = [self.topic_sizes[i]
                     for i in range(0, len(self.topic_sizes))]
        ...
        # assign documents to topic
        self.doc_top_reduced, self.doc_dist_reduced, self.doc_dist_reduced_all = self._calculate_documents_topic(self.topic_vectors_reduced,
    ...
    def search_documents_by_vector(self, vector, num_docs, return_documents=True, use_index=False, ef=None):
        ...
        if use_index:
            ...
            index_ids, doc_scores = self.document_index.knn_query(
                vector, k=num_docs)
            index_ids = index_ids[0]
            doc_ids = np.array([self.index_id2doc_id[index_id]
                               for index_id in index_ids])
    ...
    def search_words_by_vector(self, vector, num_words, use_index=False, ef=None):
        ...
        if use_index:
            ...
            word_indexes, word_scores = self.word_index.knn_query(
                vector, k=num_words)
    ...
    def search_documents_by_topic(self, topic_num, num_docs, return_documents=True, reduced=False):
        ...
        if reduced:
            ...
            topic_document_indexes = np.where(
                self.doc_top_reduced == topic_num)[0]
    ...
    def search_documents_by_keywords(self, keywords, num_docs, keywords_neg=None, return_documents=True,
        ...
        self._validate_num_docs(num_docs)
        keywords, keywords_neg = self._validate_keywords(
            keywords, keywords_neg)
    ...
    def similar_words(self, keywords, num_words, keywords_neg=None, use_index=False, ef=None):
        ...
        num_res = min(num_words + len(keywords) + len(keywords_neg),
    ...
    def search_topics(self, keywords, num_topics, keywords_neg=None, reduced=False):
        ...
        if reduced:
            ...
            topic_words = [self.topic_words_reduced[topic]
                           for topic in topic_nums]
            word_scores = [self.topic_word_scores_reduced[topic]
                           for topic in topic_nums]
            ...
            word_scores = [self.topic_word_scores[topic]
                           for topic in topic_nums]
    ...
    def search_documents_by_documents(self, doc_ids, num_docs, doc_ids_neg=None, return_documents=True,
        ...
        if self.embedding_model == 'doc2vec':
            ...
            doc_vecs_neg = [self.document_vectors[ind]
                            for ind in doc_indexes_neg]
            combined_vector = self._get_combined_vec(doc_vecs, doc_vecs_neg)
    ...
    def prediction(self, documents, use_corpus_file=False, min_count=50,
                   keep_documents=True, embedding_model='doc2vec', use_cached=False, dataset_type='all'):
        ...
        if not all((isinstance(doc, str) or isinstance(doc, np.str_)) for doc in documents):
            raise ValueError("Documents need to be a list of strings")
        if keep_documents:
            self.documents_test = np.array(documents, dtype="object")
        else:
        ...
        if embedding_model == 'doc2vec':
            ...
            self.document_vectors_test = self._embed_documents(
                documents, dataset_type, use_cached)
...
#         t = self.umap_cls.transform(self._get_document_vectors_test())

#         self.doc_top_test_hdb, self.doc_top_strengths_hdb = hdbscan.approximate_predict(self.cluster_cls, t)

        self.doc_top_test, self.doc_dist_test, self.doc_dist_test_all = self._calculate_documents_topic(
            self.topic_vectors, self._get_document_vectors_test())
        self._reorder_topics_pred()

    def _get_document_vectors_test(self, norm=True):
        return self.document_vectors_test

    def tf_idf(self, word_count=15):
        res_df = pd.DataFrame(columns=['Document', 'Topic'])
        res_df['Document'] = self.doc_df['Document']
        res_df['Topic'] = self.doc_top

        def _preprocess_text(documents: np.ndarray):
            ...
            cleaned_documents = [doc.replace("\t", " ")
                                 for doc in cleaned_documents]
            cleaned_documents = [re.sub(r'[^A-Za-z0-9 ]+', '', doc)
                                 for doc in cleaned_documents]
            cleaned_documents = [
                doc if doc != "" else "emptydoc" for doc in cleaned_documents]
            return cleaned_documents

        sizes = res_df.groupby(['Topic']).count().sort_values(
            "Document", ascending=False).reset_index()
        topic_sizes = dict(zip(sizes.Topic, sizes.Document))

        documents_per_topic = res_df.groupby(
        ...
        words = vec_model.get_feature_names()
        X = vec_model.transform(documents)
...
#         labels = list(topic_sizes.keys())
        print('*** LABELS ***\n', labels)
        print('*** CTFIDF ***\n', c_tf_idf)

        # Get top 30 words per topic based on c-TF-IDF score
        indices = c_tf_idf.argsort()[:, -word_count:]
        print('*** INDICES ***\n', indices)
